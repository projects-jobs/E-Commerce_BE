...
const verifyPayment = async (req, res, next) => {
  try {
    ...
    const order = await Order.create({
      ...
      statusHistory:   [{ status: "Processing", note: "Payment received & verified" }],
    });
    ...
    if (io) io.to(`user_${req.user._id}`).emit("notification", { message: `✅ Order #${order._id} confirmed!`, orderId: order._id });
...
const updateOrderStatus = async (req, res, next) => {
  try {
    ...
    await User.findByIdAndUpdate(order.user._id, {
      $push: { notifications: { message: `${status === "Shipped" ? "🚚" : status === "Delivered" ? "✅" : "📦"} Order #${order._id} is now ${status}`, read: false } },
    });
...
const adminStats = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, stats: { totalOrders, totalRevenue: revenueAgg[0]?.total || 0, totalUsers, totalProducts, pendingOrders, deliveredOrders, monthly } });
  } catch (err) { next(err); }
};
